...
/* The Pico port uses the 16-bit renderer originally written for the
   Game & Watch targets (the #else branches are a 24-bit desktop path). */
#if GWENESIS_PICO != 0 || GNW_TARGET_MARIO != 0 || GNW_TARGET_ZELDA != 0
#define GWENESIS_16BIT_RENDER 1
#else
#define GWENESIS_16BIT_RENDER 0
#endif
...
#include <stdint.h>
extern unsigned char *VRAM; // allocated by the port (port/buffers.c)
...
/* Channel spread/pack depends on the display pixel format (see
   gwenesis_port.h); the 565 variant also widens upstream's 0xe800 red mask
   to the full field so shadowed pixels don't lose their low red bit. */
#define CONV(b)   GWENESIS_PIX_CONV(b)
#define SPACE(c)  GWENESIS_PIX_SPACE(c)

__attribute__((optimize("unroll-loops"))) static void
GW_SRAM_FUNC(blit_4to5_line)(uint16_t *in, uint16_t *out) {
...
void GW_SRAM_FUNC(gwenesis_vdp_render_line)(int line)
{
  ...
  if (screen_width == 320) {
    ...
    if (MODE_SHI) {
      for (int x = 0; x < screen_width; x++) {
        ...
        if ((plane & 0xC0) < (sprite & 0xC0)) {
          ...
          case 0x3E:
            screen_buffer_line[x] = GWENESIS_PIX_HIGHLIGHT_MASK | CRAM565[plane] >> 1;
            break;
    ...
    static uint16_t buffer_line_H32[256]; /* static: core0 stack is 3 KB */
    ...
    if (MODE_SHI) {
      for (int x = 0; x < screen_width; x++) {
        ...
        if ((plane & 0xC0) < (sprite & 0xC0)) {
          ...
          // Palette=3, Sprite=14 :> draw plane, force highlight
          case 0x3E:
            buffer_line_H32[x] = GWENESIS_PIX_HIGHLIGHT_MASK | CRAM565[plane] >> 1;
            break;
          // Palette=3, Sprite=15 :> draw plane, force shadow
          case 0x3F:
    ...
    static uint16_t scaled_buffer_line[320]; /* static: core0 stack is 3 KB */
