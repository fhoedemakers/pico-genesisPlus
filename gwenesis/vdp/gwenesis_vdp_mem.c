...
int GW_SRAM_FUNC(gwenesis_vdp_hcounter)()
{
 ...
 ******************************************************************************/
static inline __attribute__((always_inline))
void GW_SRAM_FUNC(gwenesis_vdp_vram_write)(unsigned int address, unsigned int value)
{
  VRAM[address] = value;
...
void gwenesis_vdp_dma_fill(unsigned short value)
{
  ...
  case 0x3: // undocumented and buggy, see vdpfifotesting
    do {
      ...
      // CRAM 9-bit BGR -> display pixel format (see gwenesis_port.h)
      pixel = GWENESIS_CRAM_TO_PIXEL(fifo[3]);
    ...
    if ( src_addr & 0x800000) {
      ...
      case 0x3: // dest is CRAM
        do {
          ...
          unsigned short pixel;
          // CRAM 9-bit BGR -> display pixel format (see gwenesis_port.h)
          pixel = GWENESIS_CRAM_TO_PIXEL(value);
 ...
 ******************************************************************************/
 //static inline 
unsigned int GW_SRAM_FUNC(gwenesis_vdp_read_memory_16)(unsigned int address)
{
...
void GW_SRAM_FUNC(gwenesis_vdp_write_memory_16)(unsigned int address, unsigned int value) {
